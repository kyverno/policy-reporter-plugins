...
#
# Usage:
#   ./scripts/setup.sh              create/update the cluster and deploy
#   ./scripts/setup.sh teardown     delete the kind cluster
#
# Env overrides: CLUSTER_NAME, NAMESPACE, SERVICE_IP, IMAGE_TAG

set -euo pipefail

CLUSTER_NAME="${CLUSTER_NAME:-vap-plugin-dev}"
NAMESPACE="${NAMESPACE:-vap-plugin}"
...
subjectAltName = IP:${SERVICE_IP},DNS:vap-plugin.${NAMESPACE}.svc,DNS:vap-plugin.${NAMESPACE}.svc.cluster.local
EOF
...
kctl apply -f "$ROOT_DIR/deploy/crds/"
kctl wait --for=condition=Established --timeout=60s \
	crd/reports.openreports.io crd/clusterreports.openreports.io
...
log "building vap-plugin image with ko"
# Captures ko's resolved image reference (its one stdout line; progress
# goes to stderr, left to print live) rather than trusting the fixed
# IMAGE_TAG - that digest is what makes every rerun actually deploy what
# was just built. A fixed tag was tried first: Helm/Kubernetes saw the same
# image string on every rerun and never rolled the pod, so repeated
# `./scripts/setup.sh` runs during iterative development silently kept
# running the *first* build forever.
IMAGE_REF="$(cd "$ROOT_DIR" && KO_DOCKER_REPO=ko.local ko build . --bare --tags "$IMAGE_TAG")"
IMAGE_REPO="${IMAGE_REF%%:*}"
IMAGE_DIGEST_TAG="${IMAGE_REF##*:}"

log "loading image into kind cluster"
kind load docker-image "$IMAGE_REF" --name "$CLUSTER_NAME"

log "deploying vap-plugin via Helm"
...
helm upgrade --install vap-plugin "$ROOT_DIR/charts/vap-plugin" \
	--kube-context "$KIND_CONTEXT" \
	--namespace "$NAMESPACE" \
	--set image.repository="$IMAGE_REPO" \
	--set image.tag="$IMAGE_DIGEST_TAG" \
	--set image.pullPolicy=Never \
	--set service.clusterIP="$SERVICE_IP" \
	--set tls.existingSecret=vap-plugin-tls \
	--set config.report.reportDenied=false \
	--wait --timeout=2m
