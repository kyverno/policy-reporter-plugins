...
func (c *client) GetPolicy(ctx context.Context, resource string) (*sdk.Policy, error) {
	...
	if spec, ok := policy["spec"].(map[string]any); ok {
		details.Details = []sdk.DetailsItem{
			{Title: "Background", Value: utils.ToBoolString(spec["background"])},
			{Title: "Admission", Value: utils.ToBoolString(spec["admission"])},
			{Title: "FailurePolicy", Value: utils.ToString(spec["failurePolicy"])},
			{Title: "Mode", Value: utils.ToString(spec["validationFailureAction"])},
		}
	}
...
func NewClient(metaClient metadata.Interface, dynamicClient dynamic.Interface, kclient kyvernoV1.KyvernoV1Interface, coreClient *core.Client, cache *gocache.Cache[string, []sdk.PolicyListItem]) Client {
