...
func NewHandler(client ivpol.Client, coreAPI *core.Client) *APIHandler {
	return &APIHandler{client, coreAPI}
}

func WithAPI(client ivpol.Client, coreAPI *core.Client) server.ServerOption {
	return func(s *server.Server) error {
		return s.Register("ivpol/v1", NewHandler(client, coreAPI))
	}
}
