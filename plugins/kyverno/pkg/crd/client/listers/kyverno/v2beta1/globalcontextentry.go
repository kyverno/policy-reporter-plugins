...
type GlobalContextEntryLister interface {
	...
	List(selector labels.Selector) (ret []*kyvernov2beta1.GlobalContextEntry, err error)
	// Get retrieves the GlobalContextEntry from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kyvernov2beta1.GlobalContextEntry, error)
...
type globalContextEntryLister struct {
	listers.ResourceIndexer[*kyvernov2beta1.GlobalContextEntry]
}
...
func NewGlobalContextEntryLister(indexer cache.Indexer) GlobalContextEntryLister {
	return &globalContextEntryLister{listers.New[*kyvernov2beta1.GlobalContextEntry](indexer, kyvernov2beta1.Resource("globalcontextentry"))}
}
