...
type ClusterCleanupPolicyLister interface {
	...
	List(selector labels.Selector) (ret []*kyvernov2beta1.ClusterCleanupPolicy, err error)
	// Get retrieves the ClusterCleanupPolicy from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kyvernov2beta1.ClusterCleanupPolicy, error)
...
type clusterCleanupPolicyLister struct {
	listers.ResourceIndexer[*kyvernov2beta1.ClusterCleanupPolicy]
}
...
func NewClusterCleanupPolicyLister(indexer cache.Indexer) ClusterCleanupPolicyLister {
	return &clusterCleanupPolicyLister{listers.New[*kyvernov2beta1.ClusterCleanupPolicy](indexer, kyvernov2beta1.Resource("clustercleanuppolicy"))}
}
