...
type GlobalContextEntryLister interface {
	...
	List(selector labels.Selector) (ret []*kyvernov2.GlobalContextEntry, err error)
	// Get retrieves the GlobalContextEntry from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*kyvernov2.GlobalContextEntry, error)
...
type globalContextEntryLister struct {
	listers.ResourceIndexer[*kyvernov2.GlobalContextEntry]
}
...
func NewGlobalContextEntryLister(indexer cache.Indexer) GlobalContextEntryLister {
	return &globalContextEntryLister{listers.New[*kyvernov2.GlobalContextEntry](indexer, kyvernov2.Resource("globalcontextentry"))}
}
